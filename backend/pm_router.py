...
# ─────────────────────────────────────────────────────────────────────
# F4 · GET /api/pm/me — identidad PM del usuario autenticado
# ─────────────────────────────────────────────────────────────────────
@pm_router.get("/me")
async def pm_me(user: UserInfo = Depends(get_current_user)):
    """Devuelve identidad PM del usuario autenticado. 404 si no es PM activo."""
    if user is None:
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow("""
            SELECT pm.id_pm, pm.nombre, pm.especialidad, pm.nivel
            FROM compartido.pmo_project_managers pm
            WHERE pm.id_usuario_rbac = $1 AND pm.activo = TRUE
        """, user.id_usuario)
    if not row:
        raise HTTPException(status_code=404, detail="Usuario no es PM activo")
    return {
        "pm_code": row["id_pm"],
