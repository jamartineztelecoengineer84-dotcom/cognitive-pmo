...
## Sección 4 — F-ARQ02-13: 56 menciones legacy huérfanas en `agent_conversations.content`

### 4.1 — ¿F-ARQ02-13 == F-ARQ02-17?

Comparación explícita:

| Aspecto | F-ARQ02-13 (deuda original) | F-ARQ02-17 (anotada en sec 2.6) |
|---|---|---|
| Definición | "menciones legacy huérfanas en `agent_conversations.content` que no se mapearon en F1" | "IDs huérfanos del map F1: filas con menciones legacy que nunca se renumeraron porque sus tickets ya no existían en `incidencias_run` cuando se hizo F1" |
| Conteo declarado | 56 (según B.2 baseline) | 56 (según sec 2.6) |
| Scope | menciones `INC-[0-9]{8}-[A-F0-9]+` con `ticket_id IS NULL` post-backfill | idem |
| Tabla | `agent_conversations` | `agent_conversations` |
| Causa raíz | tickets borrados antes de F1 / fuera del universo de migración | tickets borrados antes de F1 / fuera del universo de migración |

**Veredicto**: **son la MISMA deuda**. F-ARQ02-17 fue una redescripción inadvertida que abrí en sección 2.6 al cerrar B.2 sin caer en que ya existía F-ARQ02-13. **Cierro F-ARQ02-17 como DUPLICADA de F-ARQ02-13** en este recon (no hay commit asociado a F-17 — fue sólo una nota en markdown). El nombre canónico que sobrevive es **F-ARQ02-13**.

### 4.2 — Conteo post-B.2

```sql
SELECT count(*) FROM agent_conversations
WHERE content ~ 'INC-[0-9]{8}-[A-F0-9]+' AND ticket_id IS NULL;
→ 56
```

Coincide exactamente con el baseline esperado. B.2 no recuperó ninguna (ya documentado en sec 2.6: el bug `regexp_match` singular no era la causa real).

### 4.3 — Distribución por agent_id y por día
...
**Lectura**: distribución concentradísima en **3 días específicos** (21–22 marzo + 1 abril), todos **anteriores al cierre de F1** (que ocurrió ~07-abril según el header de la migración). Son tres ráfagas históricas de testing/scenarios. AG-002 y AG-001 dominan (50/56) → cadenas Dispatcher → Resource Manager. Ningún huérfano en agentes BUILD ni en agentes posteriores a 2026-04-01.

### 4.4 — Sample 3 filas + extracción de IDs legacy

| id | agent | role | created | content[:200] | IDs extraídos |
|---|---|---|---|---|---|
| `fceeb2fb...` | AG-002 | assistant | 2026-04-01 10:49:43 | `Procesando asignación de recursos para incidencia crítica SWIFT INC-20260401-7E9E... "ticket_id": "INC-20260401-7E9E"...` | `['INC-20260401-7E9E', 'INC-20260401-7E9E']` |
| `7abdbc13...` | AG-002 | user | 2026-04-01 10:49:43 | `Resultado del Dispatcher: Voy a procesar esta incidencia crítica de SWIFT...` | (mención más allá del char 250) |
| `1d3a054d...` | AG-001 | assistant | 2026-04-01 10:49:05 | `Voy a procesar esta incidencia crítica de SWIFT... Ahora creo la incidencia...` | (mención más allá del char 250) |

**Verificación contra los CSVs del map F1**:

```
total_legacy_ids_distintos_en_huerfanos: 20
map_F1_total_old_ids:                    37
intersección huérfanos ∩ map:             0
```

**Cero intersección.** Los 20 IDs legacy distintos que aparecen en las 56 filas huérfanas (`INC-20260321-*`, `INC-20260322-*`, `INC-20260401-7E9E`, `INC-20260401-A7CF`, `INC-20260401-CFC9`, etc.) **no están en ninguno de los 2 CSVs del map F1**. Son huérfanos genuinos: tickets que existieron en algún momento (los agentes los menciona explícitamente con datos estructurados tipo `"ticket_id": "INC-..."`), pero fueron **borrados antes** de la migración F1 — probablemente seeds/scenarios purgados en cleanups intermedios o en F4 cleanup. **No hay bug oculto en B.2**: el mapping F1 simplemente no los contemplaba porque ya no existían cuando se generó.

### 4.5 — ¿Hay un mapping alternativo?

**Búsqueda de tablas legacy/backup/archive**:

```sql
SELECT table_name FROM information_schema.tables
WHERE table_schema='public'
  AND (table_name ILIKE '%legacy%' OR ILIKE '%backup%' OR ILIKE '%archive%'
       OR ILIKE '%hist%' OR ILIKE '%old%' OR ILIKE '%deleted%');
```

| table_name | ¿Útil? |
|---|---|
| `build_stakeholders` | NO — falso positivo del LIKE (`stake**hold**ers`) |
| `cmdb_demand_history` | NO — falso positivo (`demand_**hist**ory`, datos de capacity Prophet, no incidencias) |

**Búsqueda de columnas `old_*` / `legacy_*` / `previous_*` en `incidencias_run`**:

```sql
SELECT column_name FROM information_schema.columns
WHERE table_name='incidencias_run'
  AND column_name ILIKE '%old%' OR ILIKE '%legacy%' OR ILIKE '%previous%';
→ (ninguna)
```

**Conclusión**: **no existe mapping alternativo**. Ni tabla histórica de incidencias borradas, ni columna `old_ticket_id` en `incidencias_run`, ni dump intermedio guardado. Los 20 IDs legacy son **terminalmente irrecuperables** vía mapeo. La única información sobre ellos es el `content` literal de las 56 filas que los mencionan.

### 4.6 — Veredicto en 4 líneas

**(a) ¿F-13 y F-17 son la misma?** **SÍ, idénticas.** F-ARQ02-17 fue una redescripción inadvertida en sec 2.6 al cerrar B.2; el nombre canónico es **F-ARQ02-13** y F-17 queda cerrada como duplicada en este mismo recon (sin commit, era sólo una nota markdown).

**(b) ¿Recuperables o huérfanos terminales?** **Huérfanos terminales.** Los 20 IDs legacy distintos no están en ningún CSV del map F1, no hay tabla `*_legacy`/`*_backup`/`*_archive`, no hay columna `old_ticket_id` en `incidencias_run`, y ningún ticket actual contiene metadatos que apunten a ellos. La única evidencia de su existencia es el `content` literal de las 56 filas que los mencionan; no hay forma honesta de mapearlos a tickets vivos.

**(c) ¿Valor analítico/operativo?** **Bajo a nulo.** Son 56 mensajes históricos concentrados en 3 días (21–22 mar + 1 abr) de cadenas Dispatcher → Resource Manager (AG-001/002 = 50/56) sobre tickets de scenarios/seeds que ya no existen. No hay tickets vivos a los que vincular, las conversaciones no son operativamente recuperables (sus tickets están borrados), y los agentes implicados no son auditables retroactivamente porque sus inputs/outputs ya no tienen contexto referenciable. Para un dashboard de "% conversaciones con ticket_id poblado" estas 56 sólo añaden ruido en el denominador.

**(d) Recomendación**: **dejar como NULL aceptado + documentar la exclusión** en cualquier futura métrica de cobertura. Acciones concretas:
1. **NO borrar las filas** — son auditoría histórica legítima del comportamiento de los agentes en esos días, aunque los tickets ya no existan. El borrado destruiría telemetría sin recuperar nada.
2. **NO usar sentinel** tipo `'LEGACY-HUERFANO-FX'` — rompería `test_ningun_ticket_id_fantasma` (que exige que todo `ticket_id NOT NULL` exista en `incidencias_run`) salvo añadiendo un caso especial al test, lo cual contamina la regla.
3. **NO reabrir backfill creativo** — sin mapping no hay forma honesta. Cualquier heurística (mapear por timestamp al ticket más cercano, o por contexto del session_id) genera atribuciones falsas que serían peores que NULL.
4. **SÍ añadir vista/función helper** `agent_conversations_cobertura` (opcional, ~5 LOC) que excluya estas 56 filas del denominador en métricas de cobertura, vía un filtro estable: `WHERE NOT (ticket_id IS NULL AND content ~ 'INC-[0-9]{8}-[A-F0-9]+' AND created_at < '2026-04-07')`. Documentar como "huérfanos pre-F1, no recuperables".
5. **Marcar F-ARQ02-13 como CERRADA-POR-IRRECUPERABILIDAD** con esta sec 4 como evidencia. Cero código ejecutivo, optionalmente la vista helper si el usuario la quiere.

**Implicación para B.4 ejecutiva**: si el usuario decide continuar con (4) la vista helper, son ~5 LOC en una migración SQL idempotente + 1 test que asserte que la vista excluye exactamente 56 filas. Si prefiere cerrar sin vista, B.4 es 1 commit de docs (markdown only) cerrando F-13 + duplicado F-17.

### 4.7 — Cierre formal F-ARQ02-13 + cierre Bloque B

**F-ARQ02-13 CERRADA-POR-IRRECUPERABILIDAD — 2026-04-09** con vista helper `agent_conversations_cobertura` (`database/arq02_deudaB4_cobertura.sql`, idempotente vía `CREATE OR REPLACE VIEW`). La vista excluye exactamente las 56 filas con menciones legacy huérfanas pre-F1 mediante el filtro estable `NOT (ticket_id IS NULL AND content ~ 'INC-[0-9]{8}-[A-F0-9]+' AND created_at < '2026-04-07')`. Verificación post-aplicación: `count(agent_conversations) - count(agent_conversations_cobertura) = 1636 - 1580 = 56`. Las filas siguen vivas en la tabla cruda (auditoría legítima del comportamiento de los agentes en 2026-03-21/22 y 2026-04-01); la vista se usa en cualquier métrica de "cobertura ticket_id" para no contaminar el denominador con huérfanos terminales. **F-ARQ02-17 cerrada como duplicada de F-ARQ02-13** (era una redescripción inadvertida en sec 2.6).

**Cierre Bloque B**: las 4 deudas atacadas en este bloque (F-12, F-10, F-11, F-13) quedan resueltas con 4 commits acumulados (B.1 `6a23262`, B.2 `6de0a35`, B.3 `a28684a`, B.4 pendiente al cerrar este recon) + 1 test integrador end-to-end que valida las 3 invariantes en una sola pasada. Suite completa: 81 passed / 3 failed conocidos (los 3 históricos F-ARQ02-01 sin relación con Bloque B). Deudas derivadas abiertas para Bloque C+: F-ARQ02-15 (silent miss-attribution en 69 multi-mention pobladas) y F-ARQ02-16 (AG-002 downstream sin prefijo TICKET:).
